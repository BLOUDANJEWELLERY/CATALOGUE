...
export default function Catalogue({ items }: CatalogueProps) {
  ...
  const [nextModelNumber, setNextModelNumber] = useState<number>(items.length + 1);
...
const handleFileChange = async (e: ChangeEvent<HTMLInputElement>, itemId: string) => {
  ...
  try {
    ...
    const uploadData: { assetId: string; assetUrl?: string; error?: string } = await uploadRes.json();
    if (!uploadRes.ok) throw new Error(uploadData.error || "Upload failed");

    if (itemId === "blank") {
      // Collect data from new item form
      if (newItemSizes.length === 0) throw new Error("Select at least one size");
      if (newItemSizes.includes("Adult") && !newItemWeightAdult) throw new Error("Enter Adult weight");
      if (newItemSizes.includes("Kids") && !newItemWeightKids) throw new Error("Enter Kids weight");
      ...
      const createData: { success?: true; doc?: CatalogueItem; error?: string } = await createRes.json();
      if (!createRes.ok) throw new Error(createData.error || "Failed to create item");
    } else {
      // Update existing item image only
      const updateRes = await fetch(`/api/updateItem/${itemId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ assetId: uploadData.assetId }),
      });
      const updateData: { success?: true; doc?: CatalogueItem; error?: string } = await updateRes.json();
